...
/// Every SF Symbol name the icon browser offers, in Apple's own order.
///
/// Checked in rather than read at runtime, because macOS publishes no API
/// that enumerates SF Symbols. The names do sit on disk, in
/// `/System/Library/CoreServices/CoreGlyphs.bundle`, but that is a private
/// path holding private plists — reading it in the app would put a browser
/// full of empty cells one OS update away. Generated once from those plists,
/// validated once, and then it cannot move under us.
///
/// The 9184 names that bundle lists became these 4173, in this order:
///
/// - Dropped the 605 names in `symbol_restrictions.strings`. Those carry
///   Apple's "may only be used to refer to" restriction — iCloud, HomePod,
///   SharePlay, Apple Intelligence — and a tab icon is not what they are
///   for.
/// - Dropped the localized and right-to-left variants (`.ar`, `.hi`,
///   `.rtl`, and the rest of the language and script suffixes). They draw
///   the same idea in a script the name does not hint at, so in a grid they
///   read as duplicates of the row above.
/// - Kept only names that `name_availability.plist` says arrive by macOS
///   13.0, the app's deployment target. This is what guarantees no cell
///   here draws an empty box on the oldest macOS this app runs on;
///   validating on the build machine only proves a name works on the build
///   machine.
/// - Where Apple has since renamed a symbol, shipped the *old* name. 337
///   entries are that case, and skipping it was the first thing that went
///   wrong: `symbol_order.plist` lists only today's canonical names, so
///   `doc.text` is filed as `text.document` (2024) and `terminal` as
///   `apple.terminal` (2023). Filtering on the canonical name's arrival
///   dropped both — glyphs that have existed since 2019 — along with the
///   rest of the document family. `name_aliases.strings` maps the old
///   spelling to the new one, and the old spelling is the one that draws on
///   macOS 13, so that is the one shipped. One name per glyph either way:
///   no name appears here twice, and no glyph does.
/// - Fed every survivor to `NSImage(systemSymbolName:accessibilityDescription:)`
///   and kept only what came back with an image. All 4173 did, which is the
///   point of doing it — `SidebarIconCatalogTests` runs the same check on
///   every build, so a hand-edited typo here fails a test instead of
///   shipping a blank cell.
///
/// One name per line in a single literal, not an array of 4173 elements:
/// the literal costs the type checker nothing, and `git diff` on it reads as
/// the list it is.
